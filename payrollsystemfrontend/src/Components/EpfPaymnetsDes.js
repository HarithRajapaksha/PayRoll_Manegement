...
function EpfPaymnetsDes() {
  ...
  const generatePDF = () => {
    ...
    if (!epfData.length || !monthName || !selectedYear) {
    ...
    pdf.setFontSize(14);
    pdf.setFont('helvetica', 'bold');
    pdf.text(`EPF Report - ${monthName} ${selectedYear}`, 105, 43, { align: 'center' });
    pdf.setFontSize(12);
    

    // Prepare table data
    const tableBody = epfData.length > 0
      ? epfData.map((item) => [
          item.employeeId || 'N/A',
          item.employeeName || 'N/A',
          item.role || 'N/A',
          `Rs. ${(item.basicSalary || 0).toLocaleString()}`,
          `Rs. ${(item.Employee || 0).toLocaleString()}`,
          `Rs. ${(item.Employer || 0).toLocaleString()}`,
          `Rs. ${((item.Employee || 0) + (item.Employer || 0)).toLocaleString()}`
        ])
      : [['N/A', 'No EPF data available', 'N/A', 'Rs. 0', 'Rs. 0', 'Rs. 0', 'Rs. 0']];

    // Prepare summary data as a table
    const summaryBody = [
      ['Total Records', epfData.length.toString()],
      ['Total Employer Contribution', `Rs. ${totalEmployer.toLocaleString()}`],
      ['Total Employee Contribution', `Rs. ${epfData.reduce((sum, item) => sum + (item.Employee || 0), 0).toLocaleString()}`],
      ['Grand Total EPF', `Rs. ${(totalEmployer + epfData.reduce((sum, item) => sum + (item.Employee || 0), 0)).toLocaleString()}`],
    ];

    // Generate EPF data table
    autoTable(pdf, {
      startY: 55,
      head: [['Employee ID', 'Employee Name', 'Role', 'Basic Salary', 'Employee Contribution', 'Employer Contribution', 'Total EPF']],
      body: tableBody,
      theme: 'grid',
      headStyles: {
    ...
    // Save PDF
    pdf.save(`EPF_Report_${monthName}_${selectedYear}.pdf`);
  };
      ...
      <div className="container mt-4">
        <div className="row">
          <div className="col-12">
            <h2 className="mb-4">EPF Payment Report</h2>
              ...
              <>
                ...
                <div className="card mb-4">
                  <div className="card-header">
                    <h4>EPF Payment for {getMonthName(selectedMonth)} {selectedYear}</h4>
                  </div>
                  <div className="card-body">
